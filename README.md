...
All three inputs are public, free, and covered by the Open Government Licence v3.0. This repo does **not** redistribute the raw files (they're large and the license is best honoured by linking to the origin) — download them from:
...
- Only **urban** MSOAs are analysed. Rural MSOAs have near-zero minority populations in most of the country and would flatten the gradient distribution toward zero, obscuring the clustering the analysis is trying to detect.
- Adjacency is determined by polygon boundary contact (`geopandas.touches()`), accelerated with a spatial index (`.sindex`) rather than a O(n²) comparison.
...
- **5% threshold is a convention, not a law of statistics.** It follows common practice in segregation-index literature to avoid small-denominator noise, but the exact cutoff (5% vs. 3% vs. 10%) varies depending on the literature and changes which LADs appear in the "high segregation" count at the margin (see the South Derbyshire case, excluded despite D = 0.654).
